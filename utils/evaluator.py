...
Mirrors pricer/evaluator.py, tuned for the 0-99 VFM scale: absolute error
bands, and R² reported alongside MAE. Every model in the ladder runs through
this same harness on the same held-out slice.
"""
...
class Tester:
    ...
    def _error_trend(self) -> None:
        ...
        ci = [
            1.96 * sd / math.sqrt(i) if i > 1 else 0.0 for i, sd in zip(x, running_stds)
        ]
        ...
        fig.add_trace(
            go.Scatter(
                ...
                customdata=ci,
                hovertemplate="n=%{x}<br>Avg Error=%{y:.2f} VFM<br>±95% CI=%{customdata:.2f}<extra></extra>",
            )
        )
